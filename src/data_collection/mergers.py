...
def import_merged_gw(season='2023-24'):
...
def get_opponent_team_name(df):
    ...
    path = os.getcwd()
    filename = 'master_team_list_updated.csv'
    ...
    df['id'] = df['season_x'].astype(str) + '_' + df['opponent_team'].astype(str)
    ...
    #rename column
    df = df.rename(columns={"team_name_y": "opp_team_name"})
    df = df.rename(columns={"fdr": "fdr_opp_team"})
    return df

def add_fdr_data(df):
    """Add FDR data from fixtures file and match with the merged df
    """
    path = os.getcwd()
    filename = 'master_team_list_updated.csv'
    team_path = join(dirname(dirname("__file__")), path, 'data', filename)
    ...
    #create id column for both df_team and df
    df['id'] = df['season'].astype(str) + '_' + df['team'].astype(str)
    df_team['id'] = df_team['season'].astype(str) + '_' + df_team['team_name'].astype(str)
    ...
    df = df.rename(columns={"fdr": "fdr_team"})
    return df
...
def export_cleaned_data(df):
    ...
    path = os.getcwd()
    filename = 'cleaned_merged_seasons1.csv'
